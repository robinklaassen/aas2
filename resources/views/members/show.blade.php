...
@section('content')
			...
			<p class="text-right">
				@if ($viewType == 'profile')
					...
					@can("editFinance", $member)
						<a class="btn btn-success" type="button" href="{{ url('/declarations') }}"
						   style="margin-top:21px;">
							Declaratie</a>
					@endcan
				...
				@elseif ($viewType == 'admin')
					...
					@can("editPassword", $member)
						<a class="btn btn-warning" type="button"
						   href="{{ url('/users', [$member->user->id, 'password']) }}"
						   style="margin-top:21px;">Nieuw wachtwoord</a>
					@endcan
					@can("delete", $member)
	...
	@if (\Auth::user()->can("showAdministrative", $member) && $member->kmg == 0)
		<div class="alert alert-dismissible alert-warning alert-important">
			...
			<p><span class="glyphicon glyphicon-exclamation-sign"></span>&nbsp;&nbsp;Dit lid heeft nog geen KMG gehad!
			</p>
		</div>
	@endif
	...
	<div class="row">
		...
		<div class="col-md-6">
			...
			<table class="table table-hover">
				...
				<tr>
					<td>Geslacht</td>
					<td>{{ Gender::fromString($member->geslacht) }}</td>
				</tr>
				@can("showPrivate", $member)
					<tr>
						<td>Adres</td>
		...
		<div class="col-md-6">
			@can("showAdministrative", $member)
				...
				<div class="row">
					<div class="col-md-7">
						<br/>
						Meest recente actie: {{$member->most_recent_action['description']}}
						({{$member->most_recent_action['points']}} pt)
					</div>
			...
			<table class="table table-hover">
				...
				@can("showAdministrative", $member)
					...
					<tr>
						<td>Account(naam)</td>
						<td>@if($member->user()->first())
								{{ $member->user()->first()->username }}
							@else
								-geen-
							@endif</td>
					</tr>
	...
	@can("showPractical", $member)
		...
		<div class="modal fade" id="fellows" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
			 aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
	...
	@if ($viewType == 'profile')
		<!-- Modal: foto uploaden -->
		<div class="modal fade" id="uploadPic" tabindex="-1" role="dialog" aria-labelledby="uploadPicLabel"
			 aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
