			...
			<p class="text-right">
				@if ($viewType == 'profile')
					...
					@can("onEvent", $participant)
						<a class="btn btn-info" type="button" href="{{ url('/profile/on-camp') }}"
						   style="margin-top:21px;">Op kamp</a>
					@endcan
				...
				@elseif ($viewType == 'admin')
					...
					@can("update", $participant)
						<a class="btn btn-primary" type="button"
						   href="{{ url('/participants', [$participant->id, 'edit']) }}" style="margin-top:21px;">Bewerken</a>
						<a class="btn btn-info" type="button"
						   href="{{ url('/participants', [$participant->id, 'on-event']) }}" style="margin-top:21px;">Op
							kamp</a>
					@endcan

					@can("changePassword", $participant)
						<a class="btn btn-warning" type="button"
						   href="{{ url('/users', [$participant->user->id, 'password']) }}" style="margin-top:21px;">Nieuw
							wachtwoord</a>
					@endcan

					@can("delete", $participant)
						<a class="btn btn-danger" type="button"
						   href="{{ url('/participants', [$participant->id, 'delete']) }}" style="margin-top:21px;">Verwijderen</a>
					@endcan
			...
			<table class="table table-hover">
				...
				@can("showPrivate", $participant)
					...
					<tr>
						...
						<td><a href="mailto:{{$participant->email_deelnemer}}">{{ $participant->email_deelnemer }}</a>
						</td>
					</tr>
					<tr>
						<td>
		...
		<div class="col-md-6">
			...
			<table class="table table-hover">
				...
				@can("showAdministrative", $participant)
					...
					<tr>
						<td>Account(naam)</td>
						<td>@if($participant->user()->first())
								{{ $participant->user()->first()->username }}
							@else
								-geen-
							@endif</td>
					</tr>
							...
							<td>
								...
								@else
									<span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"
										  data-toggle="tooltip" title="Inkomensverklaring nog niet binnen!"></span>
								@endif
							</td>
						@endunless
					</tr>
			...
			<table class="table table-hover">
				...
				<tr>
					<td>
						Mailings ontvangen
						<span class="glyphicon glyphicon-info-sign" aria-hidden="true" data-toggle="tooltip"
							  title="Dit gaat alleen om nieuwsbrieven en kortingsacties. Bij deelname aan een kamp ontvangt u altijd mail."></span>
					</td>
			...
			<table class="table table-hover">
				...
				@forelse ($participant->events()->whereIn('type', ['kamp', 'online'])->orderBy('datum_start', 'desc')->get() as $event)
					<tr>
						...
						<td>
							@if ($courseOnCamp != [])
								@foreach ($courseOnCamp[$event->id] as $course)
									{{ $course['naam'] }} <br/>
								@endforeach
							@endif
						</td>
						<td>
							@can("editParticipantCourses", [$event, $participant])
								<a href="{{ $viewType == 'admin' ? url('/participants', [$participant->id, 'edit-event', $event->id]) : url('/profile/edit-camp', $event->id)}}">
									<span class="glyphicon glyphicon-edit" data-toggle="tooltip" title="Vakken bewerken"
