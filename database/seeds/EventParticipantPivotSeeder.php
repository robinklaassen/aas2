...
class EventParticipantPivotSeeder extends Seeder
{
	...
	public function run()
	{
		...
		$participant->events()->attach(1, ['datum_betaling' => '2015-05-22', 'geplaatst' => 1]);

		// This participant is added without courses.
		$participant = Participant::find(3);
		$participant->events()->attach(1, ['datum_betaling' => '2019-12-01', 'geplaatst' => 1]);

		// online
		$participant = Participant::find(3);
		$participant->events()->attach(7, ["package_id" => 1, 'geplaatst' => 1]);
