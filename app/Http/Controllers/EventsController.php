...
class EventsController extends Controller
{
    ...
    {
        $this->authorize('editMember', [$event, $member]);
        $event->members()->updateExistingPivot($member, [
            'wissel' => $request->wissel,
            'wissel_datum_start' => $request->wissel_datum_start,
            'wissel_datum_eind' => $request->wissel_datum_eind,
        ]);
    ...
    public function editParticipantSave(Event $event, Participant $participant, Request $request)
    {
        ...
        $event->participants()->updateExistingPivot($participant->id, [
            'datum_betaling' => $request->datum_betaling,
            'geplaatst' => $request->geplaatst,
        ]);
    ...
    public function export(Event $event)
    {
        ...
        foreach ($result as $row) {
            $participantCourses[$row->participant_id][] = [
                'naam' => $row->naam,
                'info' => $row->info,
            ];
        }
        ...
        foreach ($participants as $p) {
            if (in_array($p->klas . $p->niveau, ['4VMBO', '5HAVO', '6VWO'], true)) {
                $stats['num_exam']++;
            }
        }
    ...
    public function send(Event $event)
    {
        ...
        foreach ($ids as $id) {
            $event->participants()->updateExistingPivot($id, [
                'geplaatst' => 1,
            ]);
        }
